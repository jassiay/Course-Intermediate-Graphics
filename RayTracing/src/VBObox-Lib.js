...
VBObox0.prototype.appendWireCube = function(){
	...
	var vertSet = [ 0,0,0,1, 	0.3, 1.0, 1.0, 1.0,
...
// Render current VBObox contents.
  ...
  // ----------------------------Draw the contents of the currently-bound VBO:
	...
	mat4.copy(modelMat, this.mvpMat); 
	gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, 	this.mvpMat);	// send data from Javascript.
	...
	for(var i = 0; i< g_currentScene.items.length; i++){
    var temp = mat4.create();
        mat4.copy(temp, modelMat);
        mat4.multiply(temp, temp,  g_currentScene.items[i].world2model);
        
        gl.uniformMatrix4fv(this.u_mvpMatLoc,	false, 	temp);	// send data from Javascript.
        switch(g_currentScene.items[i].shapeType){
            case RT_GNDPLANE:
                gl.drawArrays(gl.LINES, this.bgnGrid, this.bgnDisk - this.bgnGrid);
                break;
            case RT_DISK:
                gl.drawArrays(gl.LINES, this.bgnDisk, this.bgnSphere - this.bgnDisk); 
                break;
            case RT_SPHERE:
                gl.drawArrays(gl.LINE_STRIP, this.bgnSphere, this.bgnCube - this.bgnSphere);
                break;    
            case RT_BOX:
                gl.drawArrays(gl.LINE_STRIP, this.bgnCube, this.vboVerts - this.bgnCube);
                break;
        }
	  }
}
